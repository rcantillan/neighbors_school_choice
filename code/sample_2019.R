...
# Load files
  student_main <- read.csv2("./data/original datasets/SAE/SAE_2019/B1_Postulantes_etapa_regular_2019_Admisión_2020_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")
  student_complementary <- read.csv2("./data/original datasets/SAE/SAE_2019/B2_Postulantes_etapa_complementaria_2019_Admisión_2020_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")

  application_main <- read.csv2("./data/original datasets/SAE/SAE_2019/C1_Postulaciones_etapa_regular_2019_Admisión_2020_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")
  application_complementary <- read.csv2("./data/original datasets/SAE/SAE_2019/C2_Postulaciones_etapa_complementaria_2019_Admisión_2020_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")

  enrolment_2020 <- read.csv("./data/original datasets/matricula-por-estudiante/Matricula-por-estudiante-2020/20200921_Matrícula_unica_2020_20200430_WEB.csv", header = TRUE, sep= ";", encoding="UTF-8")

# Merge files of student with their application using "mrun" (we do not include students who only )
  sample_2019 <- student_main %>% left_join(application_main, by = "mrun") %>%
                 select(-cod_nivel.y) %>%
...
# STEP 1: We keep only applicants who live in Santiago Metropolitan Area (34 municipalities)
  ...
  sample_sf_2019 <- st_transform(sample_sf_2019, 3395) # Set SCR with meter metrics
  cities <- st_transform(cities, 3395) # same SCR with meter metrics

  sample_sf_2019 <- st_join(sample_sf_2019, cities, join = st_intersects) # make the intersection. st_within returns the same results, but it is slower.
  
  sample_sf_2019 <- sample_sf_2019 %>% 
  ...
  sample_2019 <- left_join(sample_sf_2019, sample_2019, by="mrun") # merge with the national sample and keep only applicant in Santiago
...
# STEP 4: we drop students who do not have enrolled school in the next year and those enrolled in specific programs
  ...
  sample_2019 <- left_join(sample_2019, enrolment_2020, by = "mrun") %>%
...
# STEP 5: keep only relevant variables and export file
  ...
  sample_2019$cohort <- 2019 
   
  write.csv(sample_2019, "./data/final datasets/sample_2019.csv")
