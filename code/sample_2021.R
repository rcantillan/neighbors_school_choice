...
# Load files
  student_main <- read.csv2("./data/original datasets/SAE/SAE_2021/B1_Postulantes_etapa_regular_2021_Admisión_2022_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")
  student_complementary <- read.csv2("./data/original datasets/SAE/SAE_2021/B2_Postulantes_etapa_complementaria_2021_Admisión_2022_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")

  application_main <- read.csv2("./data/original datasets/SAE/SAE_2021/C1_Postulaciones_etapa_regular_2021_Admisión_2022_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")
  application_complementary <- read.csv2("./data/original datasets/SAE/SAE_2021/C2_Postulaciones_etapa_complementaria_2021_Admisión_2022_PUBL.csv", header = TRUE, sep= ";", encoding="UTF-8")

  enrolment_2022 <- read.csv("./data/original datasets/matricula-por-estudiante/Matricula-por-estudiante-2022/20220908_Matrícula_unica_2022_20220430_WEB.csv", header = TRUE, sep= ";", encoding="UTF-8")

# Merge files of student with their application using "mrun" (we do not include students who only )
  sample_2021 <- student_main %>% left_join(application_main, by = "mrun") %>%
                 select(-cod_nivel.y) %>%
...
# STEP 1: We keep only applicants who live in Santiago Metropolitan Area (34 municipalities)
  ...
  sample_sf_2021 <- st_transform(sample_sf_2021, 3395) # Set SCR with meter metrics
  cities <- st_transform(cities, 3395) # same SCR with meter metrics

  sample_sf_2021 <- st_join(sample_sf_2021, cities, join = st_intersects) # make the intersection. st_within returns the same results, but it is slower.
  
  sample_sf_2021 <- sample_sf_2021 %>% 
  ...
  sample_2021 <- left_join(sample_sf_2021, sample_2021, by="mrun") # merge with the national sample and keep only applicant in Santiago
...
# STEP 4: we drop students who do not have enrolled school in the next year and those enrolled in specific programs
  ...
  sample_2021 <- left_join(sample_2021, enrolment_2022, by = "mrun") %>%
...
# STEP 5: keep only relevant variables and export file
  ...
  sample_2021$cohort <- 2021 
   
  write.csv(sample_2021, "./data/final datasets/sample_2021.csv")
